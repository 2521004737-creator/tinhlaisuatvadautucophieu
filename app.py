...
if st.sidebar.button("📊 Tính Toán Kết Quả", type="primary"):
    with st.spinner("Đang tải và xử lý dữ liệu sạch..."):
        ...
        if not data.empty:
            ...
            # 🔥 BIỆN PHÁP DIỆT ĐOẠN ĐI NGANGẢO:
            # Chỉ giữ lại những ngày thực sự có giao dịch trên thị trường (Volume > 0)
            ...
            if not data.empty:
                ...
                if not prices.empty:
                    # TÍNH TOÁN SỐ NĂM THỰC TẾ ĐẦU TƯ KHẢ DỤNG (SAU KHI CẮT ĐOẠN ẢO)
                    so_thang_thuc_te = len(prices)
                    so_nam_thuc_te = so_thang_thuc_te / 12
                    
                    if so_nam_thuc_te < (K_nam - 0.5):
                        st.warning(f"⚠️ Mã {ticker} chỉ có dữ liệu giao dịch thực tế trong **{so_nam_thuc_te:.1f} năm** ({so_thang_thuc_te} tháng). Hệ thống đã tự động cắt bỏ đoạn dữ liệu đi ngang vô nghĩa trước đó để tính toán chính xác.")
                    else:
                    ...
                    col_cagr1.metric(
                        ...
                        f"{cagr_stock:.2f}% / năm",
                        help="Tốc độ tăng trưởng kép hàng năm của thị giá tài sản dựa trên số năm dữ liệu giao dịch thực tế."
                    )
                ...
                st.error("Không tìm thấy dữ liệu giao dịch thực tế (Volume > 0) cho mã này trong khoảng thời gian đã chọn.")
        else:
